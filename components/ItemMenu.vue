...
<template>
      ...
      <template #item="{ item }">
        <div class="grid grid-cols-2 gap-4 ">
          ...
          <template v-if="item.label === 'Chart' || item.label === 'All'">
            ...
            <ItemCard title="Radial Chart" description="An image that can be edit to shrink in a bento box" icon="i-tdesign-chart-radial" @click="setComponent(ImageSettings)" />
  ...
  <div v-else class="flex flex-col px-4 py-5 sm:p-6">
    <component :is="componentName" @return="componentName = null" />
  </div>
</template>
