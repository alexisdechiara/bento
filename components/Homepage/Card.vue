  ...
  >
    <UCard :ui="ui">
      <div :class="ui.container">
        <NuxtLink v-if="to" v-bind="nuxtLinkProps" class="focus:outline-none" tabindex="-1">
...
const omittedProps = new Set(['title', 'description', 'icon', 'color', 'orientation', 'ui', 'class'])
const nuxtLinkProps = computed(() => {
  const _props: Record<string, any> = {}

  for (const prop in props) {
    // @ts-ignore
    if (omittedProps.has(prop) || !props[prop]) { continue }
    // @ts-ignore
